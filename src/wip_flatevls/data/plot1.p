...
set terminal cairolatex svg size 5,3
load 'settings.p'
...
# ------------ PARAMETERS ----------------
array DELTA[6] = [0.05, 0.1, 0.2, 0.5, 1, 2]
...
array MEAN_PHASE3[6]
do for [j=1:6] {
    fname = 'flat'.fbuild(DELTA[j], QC[1], EPS[2], P0[1]).'.txt' 
    stats fname\
...
do for [j=1:6] {
    fname = 'flat'.fbuild(DELTA[j], QC[2], EPS[2], P0[1]).'.txt' 
    stats fname\
        using 10 every ::1::1 prefix 'pmin' noout
    stats fname \
        using 10 every ::2::2 prefix 'pmax' noout
...
do for [j=1:6] {
    fname = 'flat'.fbuild(DELTA[j], QC[3], EPS[2], P0[1]).'.txt' 
    stats fname\
        using 10 every ::1::1 prefix 'pmin' noout
    stats fname \
        using 10 every ::2::2 prefix 'pmax' noout
...
plot DELTA u (DELTA[$1]):(MEAN_PHASE[$1]) w lp ls 1 t '$q_c=$'.str(QC[1]), \
    DELTA u (DELTA[$1]):(MEAN_PHASE2[$1]) w lp ls 2 t '$q_c=$'.str(QC[2]), \
    DELTA u (DELTA[$1]):(MEAN_PHASE3[$1]) w lp ls 3 t '$q_c=$'.str(QC[3]) 
