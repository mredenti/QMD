...
if __name__ == '__main__':
    ...
    DELTA, ALPHA = 0.05, 0.5 # delta 0.1# CHANGE PARAMETERS
    NEQS = 2
    ...
    FNAME_OBS = './data/observablesfinaltimedelta%.4falpha%.4feps%.4fp%d.txt' \
            %(DELTA, ALPHA, EPS, p) # TO CHANGE
    ...
    wave = run_coupled(time_forward, space, solver_coupled, wave, potential, FNAME_OBS)
    
    f = open(FNAME_OBS, 'w')
    header = 'mean_x_up \t mean_x_down \t mean_p_up \t mean_p_down \
    ...
    data = np.array([mean_x_up, mean_x_down,
